    ...
    <div class="container">
        ...
        <form action="{{ route('presence.update', ['id' => $seance->id, 'presence_id' => $presence->id]) }}" method="POST">
            ...
            @endif
                ...
                <div class="form-group">
                    <label for="heure_debut">heure de début la séance</label>
                    <input type="text" class="form-control" id="heure_debut" value="{{ $seance->heure_debut }}" disabled>
                    <input type="hidden" name="id_seance" value="{{ $seance->id }}">
                </div>
                <div class="form-group">
                    <label for="heure_fin">heure de fin la séance</label>
                ...
                <div class="form-group">
                    ...
                    <input type="date" class="form-control" id="date" name="date" value="{{ $presence->date }}" required>
                </div>
    
                <div class="form-group">
                    <label for="heure_arrivee">Heure d'arrivée</label>
                    <input type="time" class="form-control" id="heure_arrivee" name="heure_arrivee" value="{{ $presence->heure_arrivee }}" required>
                </div>
    
                <div class="form-group">
                    <label for="heure_depart">Heure de départ</label>
                    <input type="time" class="form-control" id="heure_depart" name="heure_depart" value="{{ $presence->heure_depart }}">
                </div>
    
                <div class="form-group">
                    <label for="statut">Statut</label>
                    <input class="form-control" id="statut" value="{{ $presence->statut->nom }}" name="id_statut" readonly>
                </div>
    
                <div class="form-group" id="motif-container" style="{{ $presence->statut->nom === 'Absent(e)' ? 'display: block;' : 'display: none;' }}">
                    <label for="motif">Motif</label>
                    <input type="text" class="form-control" id="motif" name="motif" value="{{ $presence->motif }}">
                </div>
            ...
            <script>
                function updateStatut() {
                    ...
                    var heureArrivee = document.getElementById('heure_arrivee').value ? new Date('1970-01-01T' + document.getElementById('heure_arrivee').value) : null;
                    ...
                    if (datePresence === dateSeance) {
                        if (heureArrivee) {
                            var heureDebutPlus15 = new Date(heureDebut);
                            heureDebutPlus15.setMinutes(heureDebut.getMinutes() + 15);
                            
                            if (heureArrivee <= heureDebutPlus15) {
                                statut = 'Présent(e)';
                            } else if (heureArrivee > heureDebutPlus15 && heureArrivee <= heureFin) {
                                statut = 'Retard';
                            } else if (heureArrivee > heureFin) {
                                statut = 'Absent(e)';
                            }
                        } else {
                            statut = 'Absent(e)';
